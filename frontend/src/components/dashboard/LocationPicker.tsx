...
export default function LocationPicker({ location, onLocationSet }: Props) {
  ...
  const handleMapsLink = async () => {
    ...
    try {
      // Step 1: Try extracting directly from the pasted URL in the browser first
      // (instant for full URLs that already contain @lat,lng — no network round-trip needed)
      ...
      // Step 2: For short links (maps.app.goo.gl) — let OUR OWN BACKEND expand it.
      // The backend has no CORS restrictions (server-to-server request via httpx),
      // unlike third-party public CORS proxies (corsproxy.io, allorigins.win) which
      // are unreliable and frequently rate-limited or down.
      const res = await api.post('/location/parse-google-maps', { url });
      const { latitude, longitude, label } = res.data;
      const loc: LocationData = { latitude, longitude, label: label || 'Google Maps Location' };
      await api.post('/location/save', loc);
      onLocationSet(loc);
      setMapsUrl('');
      showToast({ type: 'success', title: 'Location set!', message: `${latitude.toFixed(5)}, ${longitude.toFixed(5)}` });

    } catch (err: any) {
      // Backend returns a specific, real reason in err.response.data.detail —
      // show that instead of a generic message, so the actual cause is visible.
      const detail = err?.response?.data?.detail;
      showToast({
        type: 'warning',
        title: 'Could not extract location',
        message: detail || 'Switch to "Coordinates" tab and paste the lat/lng from Google Maps instead.',
      });
    } finally {
      setLoading(false);
    }
