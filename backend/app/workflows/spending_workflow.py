...
async def data_aggregation_node(state: SpendingState) -> SpendingState:
    ...
    try:
        ...
        lab_items = []
        seen_ids = set()  # tracks every doc._id already added, across BOTH queries below

        def _expand_lab_doc(doc: dict) -> list:
            """
            Turn one selected_labs document into one-or-more line items.

            If a real per-test breakdown was saved (test_prices — added once
            this fix shipped), use the REAL per-test prices, e.g.:
                HbA1c ₹380, Thyroid Profile ₹600  (sums correctly to ₹980)

            Otherwise (older documents saved before this fix existed, which
            only ever stored one combined total for the whole visit), fall
            back to a single combined line — there is no way to recover a
            real per-test split for those, so we deliberately do NOT guess
            an even split, which would just invent incorrect numbers.
            """
            test_prices = doc.get("test_prices")
            if test_prices:
                return [{
                    "test_name": tp.get("test_name", "Lab Test"),
            ...
            return [{
                "test_name": ", ".join(tests) if tests else "Lab Test",
                "lab_name":  doc.get("lab_name", "Unknown Lab"),
        ...
        async for doc in cursor:
            ...
            lab_items.extend(_expand_lab_doc(doc))

        # Also check by month field (for older/seeded data that may lack a
        # selected_at within range, or use month-bucketing instead).
        # seen_ids (built above) is the ONLY dedup check needed here — a doc
        # already added via the date-range query above must never be added
        # again just because it also happens to carry a matching month field.
        ...
        medicine_items = []
        seen_med_ids = set()  # tracks every doc._id already added, across BOTH queries below
        ...
        async for doc in cursor:
            seen_med_ids.add(str(doc.get("_id", "")))
        ...
        # Also check by month field (for older/seeded data). seen_med_ids
        # (built above) is the ONLY dedup check needed — a doc already added
        # via the date-range query must never be added again just because it
        # also happens to carry a matching month field.
        cursor2 = db.medicine_cost_reports.find({"user_id": uid, "month": month})
        async for doc in cursor2:
            doc_id = str(doc.get("_id", ""))
            if doc_id in seen_med_ids:
                continue
            seen_med_ids.add(doc_id)
            for med in doc.get("medicines", []):
                medicine_items.append({
                    "medicine_name":     med.get("name", ""),
                    "selected_medicine": med.get("selected", med.get("name", "")),
        ...
        async for doc in cursor:
            prev_med_seen.add(str(doc.get("_id", "")))
        ...
        async for doc in cursor:
            doc_id = str(doc.get("_id", ""))
            if doc_id in prev_med_seen:
                continue
            prev_med_seen.add(doc_id)
            for m in doc.get("medicines", []):
                prev_total += m.get("price", 0) * m.get("qty", 1)
            has_prev = True

        # prev doctors — these two queries are mutually exclusive by visit_type
        # (one-time vs subscription can never both match the same document),
        # so no dedup is needed here; left as-is.
...
async def savings_suggestions_node(state: SpendingState) -> SpendingState:
    ...
    try:
        ...
        for item in state.get("lab_items", []):
            combined_name = item.get("test_name", "")
            current_price = item.get("price", 0)
            lab_name      = item.get("lab_name", "")
            if not combined_name or current_price == 0:
                continue

            # test_name may be a single test ("HbA1c") or a comma-joined
            # bundle ("HbA1c, Thyroid Profile") since one selected_labs
            # document can cover multiple tests under one combined price.
            # Split back into individual names for catalog matching —
            # we only ever stored the combined visit total, not a per-test
            # breakdown, so a suggestion is only made for single-test visits
            # where "current price" and "that lab's price for this test" are
            # directly comparable. Bundled multi-test visits are skipped here
            # rather than risk a misleading saving figure.
            individual_tests = [t.strip() for t in combined_name.split(",") if t.strip()]
            if len(individual_tests) != 1:
                continue
            test_name = individual_tests[0]
