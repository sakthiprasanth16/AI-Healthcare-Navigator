...
async def select_lab(data: SelectLabRequest, current_user=Depends(get_current_user)):
    ...
    await db.selected_labs.insert_one({
        ...
        "price": data.price,
        # Per-test breakdown — e.g. [{"test_name": "HbA1c", "price": 380}, ...].
        # Stored so a multi-test visit's combined price can be split back
        # apart correctly later (e.g. in Spending Tracker), instead of only
        # having the single combined total to work with.
        "test_prices": [tp.model_dump() for tp in data.test_prices] if data.test_prices else None,
        "selected_at": ist_now()
    })
